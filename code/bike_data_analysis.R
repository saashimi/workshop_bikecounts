...
Tilikum <-  all_bridges[[2]]  %>% 
  mutate(Lat = 45.504935) %>% 
  mutate(Lng = -122.667018)  
...
bike_ts <- ts(bike_series$total, start=2012, frequency=365)
autoplot(stl(bike_ts, s.window = 'periodic'), ts.colour = 'blue')
...
#m <-  leaflet(all_data) %>% addTiles() %>% addMarkers(
